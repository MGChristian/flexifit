...
if ($_SERVER['REQUEST_METHOD'] == "GET") {
    ...
    $stmt = $conn->prepare("SELECT `ID`, `firstName`, `lastName`, `dateCreated`, `DOB`, `gender`, `email`, `contactNo` FROM `user` WHERE `role` = 'user' && `status` = 'archived'");
    if ($stmt->execute()) {
...
{
    return "<a href='./includes/unarchive-user.php?id={$id}'><button type='button' class='data-table-button unarchive' data-target='archive-user'> <i class='fa fa-archive' aria-hidden='true'></i> </button></a>";
}
